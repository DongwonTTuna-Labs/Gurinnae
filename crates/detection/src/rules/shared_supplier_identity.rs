...
pub fn evaluate(input: &Value) -> Result<Value, EvaluationError> {
    ...
    for (index, first) in suppliers.iter().enumerate() {
        for second in suppliers.iter().skip(index + 1) {
            ...
            let first_hashes = hashes(&first["strong_identifier_hashes"])?;
            let second_hashes = hashes(&second["strong_identifier_hashes"])?;
            let shared = first_hashes.intersection(&second_hashes).count();
            let relevant = contracts.iter()
                .filter(|contract| matches!(contract["supplier_id"].as_str(), Some(id) if id == first_id || id == second_id))
                .copied()
                .collect::<Vec<_>>();
                ...
                && total >= Decimal::from(50_000_000)
            {
                let relevant_ids = relevant
                    ...
                    .collect::<Vec<_>>();
                let relevant_set: BTreeSet<_> = relevant_ids.iter().cloned().collect();
