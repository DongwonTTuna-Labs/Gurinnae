...
async fn insert_provider_turn(
    ...
    // Persist only the closed semantic request.  Selected bytes are a
    // short-lived gateway capsule and are reconstructed from the same
    // immutable snapshot immediately before each dispatch; they are never
    // mixed into the ProviderTurnV2 requestRedacted object.
    ...
    // Bind the turn to the authority-pinned prompt bytes.  Hashing the agent
    // identifier here would make every prompt provenance claim unverifiable
    // (and would silently accept prompt drift), so the registry is resolved
    // from the checked-in authority files at compile time.
...
fn output_schema_contract(agent_type: &str) -> (&'static str, &'static str, &'static str) {
    match agent_type {
        "market-researcher" => ("MarketResearchOutputV2", "market-research-output.v2", "af49da181ef6e5bf37aa43245cd2ea44e2a71a1e41ac475e4f04c17137864577"),
        "investigator" => ("InvestigatorOutputV2", "investigator-output.v2", "d1385cb045bfdffb8e49322019b735c5cf813e6207664c2ff8219f088b0c13df"),
        "skeptic" => ("SkepticOutputV2", "skeptic-output.v2", "6df71038d32529334413d7c1757a67fc0c1eba9e19d8ea9bdb6ae5d1cd1197d1"),
        "claim-drafter" => ("ClaimDraftOutputV2", "claim-draft-output.v2", "1b8118e9e01c6277863900f1bad315539a0618685ee431f1fa2ce181388272ec"),
        "citation-verifier" => ("CitationVerificationOutputV2", "citation-verification-output.v2", "9a76c04dcbca76018b5dba2eb837a80fae3fbeb3360cba4e740913de0096be94"),
        _ => ("InvestigatorOutputV2", "investigator-output.v2", "d1385cb045bfdffb8e49322019b735c5cf813e6207664c2ff8219f088b0c13df"),
    }
}
...
async fn provider_wire_request(
    ...
    // The durable ProviderTurnV2 request remains redacted and hash-bound.  The
    // gateway capsule is the short-lived provider input and must still carry
    // the actual objective and the preceding tool result so a multi-turn
    // provider can reason over what it just discovered.  Both values are
    // covered by the turn's redaction receipt above and are never persisted in
    // requestRedacted.
...
async fn selected_content_wire_refs(
    ...
    for reference in metadata {
        let key = reference.get("selectedContentObjectKey").and_then(Value::as_str)
            .ok_or_else(|| Failure::Terminal("AGENT_SOURCE_USE_INVALID", "object key".into()))?;
        let digest = reference.get("selectedContentSha256").and_then(Value::as_str)
        ...
        let mut wire = reference.as_object().cloned()
...
async fn append_research_artifact_refs(
    ...
    let Some(result) = prior_tool_result else { return Ok(()) };
    let artifacts = result.pointer("/response/artifacts").or_else(|| result.pointer("/artifacts"))
        .and_then(Value::as_array).map_or(&[][..], |value| value.as_slice());
...
async fn research_artifact_wire_ref(
    ...
    let digest = artifact.get("contentSha256").and_then(Value::as_str)
        .filter(|v| v.len() == 64 && v.bytes().all(|b| b.is_ascii_hexdigit()))
        .ok_or_else(|| Failure::Terminal("AGENT_RESEARCH_ARTIFACT_INVALID", "contentSha256".into()))?;
    ...
    for field in ["researchArtifactId", "assetId", "assetRevision", "sourceFetchId", "artifactOrdinal", "sourceAuthority", "finalOrigin", "retrievedAt", "httpStatus", "contentMediaType", "contentSizeBytes", "contentSha256", "artifactSha256", "sourceUseId", "sourceUseSha256"] {
        if let Some(value) = artifact.get(field) { wire.insert(field.to_owned(), value.clone()); }
    }
    wire.insert("selectedContentSizeBytes".into(), Value::from(bytes.len() as u64));
    wire.insert("selectedContentBytesBase64".into(), Value::String(BASE64.encode(bytes)));
    wire.insert("selectedContentSha256".into(), Value::String(digest.to_owned()));
...
mod tests {
    ...
    fn authority_prompt_registry_is_closed_and_pinned() {
        let expected = [
            ("market-researcher", "d6cd7cff54efab4febf4673694b585cc74e74498b3820df7844887cca05f2c11"),
            ("investigator", "e7654bb7abf2cad997118ac649544ea20ef23ba1e7b5598df7ec45d54a9322f6"),
            ("skeptic", "15f1a488f35bbd342e2d9ce8a8635cf7e720fccf459d7c46ee32faf3a6d4a9ef"),
            ("claim-drafter", "302357154a51844bdf976a324640632d88d781cd14a0af6d309aa6deaf97c512"),
            ("citation-verifier", "a97a37302de9e5a230f1f33ca580ddab49bb93b1ed899de72c275d7770c53492"),
        ];
