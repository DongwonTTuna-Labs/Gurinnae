...
pub(super) async fn persist_tool_call(
    ...
    sqlx::query("SELECT ops.complete_agent_tool_call_v2($1,$2,$3,CAST($4 AS char(64)),$5,$6,CAST($7 AS char(64)),CAST($8 AS char(64)),$9)")
        .bind(tool_call_id).bind(turn.run_id).bind(&call_text).bind(&result_sha256)
        .bind(result).bind(result_canonical).bind(sha256(b"response-valid"))
        .bind(transcript_sha256).bind(now).execute(&mut *tx).await.map_err(database)?;
    ...
    let summary = sqlx::query("SELECT count(*)::int AS count, encode(extensions.digest(convert_to(array_to_string(array_agg(source_use_sha256::text ORDER BY source_use_sha256),','),'UTF8'),'sha256'),'hex') AS digest FROM ops.agent_source_uses WHERE agent_run_id=$1 AND tool_call_id=$2 AND use_kind='TOOL_RESULT'")
        .bind(turn.run_id).bind(tool_call_id).fetch_one(&mut *tx).await;
    ...
    let source_use_count: i32 = match summary.try_get("count") {
        Ok(value) => value,
        Err(error) => { let _ = tx.rollback().await; if let Some(source) = pending_source_fetch { delete_orphan(state, source).await; } return Err(database(error)); }
    };
    let source_use_set_sha256: String = match summary.try_get("digest") {
        Ok(value) => value,
        Err(error) => { let _ = tx.rollback().await; if let Some(source) = pending_source_fetch { delete_orphan(state, source).await; } return Err(database(error)); }
    };
    if let Err(error) = sqlx::query("SELECT ops.finalize_agent_tool_call_source_uses($1,$2,CAST($3 AS char(64)))")
        .bind(tool_call_id).bind(source_use_count).bind(source_use_set_sha256).execute(&mut *tx).await {
        let _ = tx.rollback().await;
        if let Some(source) = pending_source_fetch { delete_orphan(state, source).await; }
