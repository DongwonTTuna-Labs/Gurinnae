use super::{Failure, ProviderTurnIdentity, State, database};
...
pub(super) async fn load_tool_snapshot(
    ...
    .bind(binding.input_snapshot_id)
    .bind(&binding.input_snapshot_sha256)
    .bind(turn.run_id)
...
async fn load_responses(
    ...
    .map(|row| {
        Ok(ResponseRecord {
            response_id: row.try_get("response_id").map_err(database)?,
            response_content_sha256: row.try_get("response_content_sha256").map_err(database)?,
        })
    })
    .collect()
}
...
async fn load_comparables(
    ...
    .map(|row| {
        Ok(ComparableRecord {
            contract_id: row.try_get("object_id").map_err(database)?,
...
async fn load_entities(
    ...
    let mut entities = rows
        .into_iter()
        .map(|row| {
            let id: Uuid = row.try_get("object_id").map_err(database)?;
            let name: String = row
    ...
    if !entities.is_empty() {
        ...
        .map_err(database)?;
        for row in aliases {
            let id: Uuid = row.try_get("entity_id").map_err(database)?;
            if let Some(entity) = entities.get_mut(&id) {
                entity.identifiers.push(EntityIdentifier {
                    kind: EntityIdentifierKind::VerifiedAlias,
                    value: row.try_get("alias").map_err(database)?,
                });
            }
        }
    }
...
async fn load_rules(
    ...
    .map(|row| {
        Ok(RuleRecord {
            rule_run_id: row.try_get("rule_run_id").map_err(database)?,
            rule_version_id: row.try_get("rule_version_id").map_err(database)?,
            result_digest: row.try_get("result_digest").map_err(database)?,
        })
    })
    .collect()
}
...
async fn load_source_artifacts(
    ...
    for row in rows {
        let content_sha256: String = row.try_get("content_sha256").map_err(database)?;
        let object_key: String = row.try_get("object_key").map_err(database)?;
        let request_kind = match row
            .try_get::<String, _>("request_kind")
            .map_err(database)?
            .as_str()
        {
        ...
        artifacts.push(SourceArtifactRecord {
            research_artifact_id: row.try_get("research_artifact_id").map_err(database)?,
            source_use_id: row.try_get("source_use_id").map_err(database)?,
            content_sha256,
...
async fn evidence_records(
    ...
    .map(|row| {
        Ok(EvidenceRecord {
            ...
            source_use_id: row.try_get("source_use_id").map_err(database)?,
            source_use_sha256: row.try_get("source_use_sha").map_err(database)?,
