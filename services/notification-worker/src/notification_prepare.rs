async fn prepare(state: &State, event: &ClaimedEvent) -> Result<(Uuid, EmailMessage), WorkerError> {
    let (to, subject, text, html) = match event.event_type.as_str() {
        ...
        "notification.response_submitted.v1" => prepare_response_submitted(state, event).await?,
...
async fn prepare_scan_completed(state: &State, event: &ClaimedEvent) -> Result<(String, String, String, String), WorkerError> {
    Ok({
...
async fn prepare_subscription_verification(state: &State, event: &ClaimedEvent) -> Result<(String, String, String, String), WorkerError> {
    Ok({
...
async fn prepare_correction_received(state: &State, event: &ClaimedEvent) -> Result<(String, String, String, String), WorkerError> {
    Ok({
...
async fn prepare_response_submitted(state: &State, event: &ClaimedEvent) -> Result<(String, String, String, String), WorkerError> {
    Ok({
let token = derived_token(
                &state.token_hmac_key,
                "response-receipt",
...
async fn prepare_correction_resolved(
    ...
    Ok((
        ...
        format!("정정 검토가 완료되었습니다. 정정 ID: {}", event.aggregate_id),
