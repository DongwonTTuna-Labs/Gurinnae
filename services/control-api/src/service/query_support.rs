...
fn query_unexpected_null() -> ServiceError {
    db(sqlx::Error::Decode(Box::new(
        sqlx::error::UnexpectedNullError,
    )))
}

fn review_readiness_status(blockers: &Value) -> &'static str {
    if blockers.as_array().is_some_and(Vec::is_empty) {
        "READY"
    } else {
        "BLOCKED"
    }
...
pub(super) async fn case_query(
    ...
    .ok_or_else(query_unexpected_null)?;
    ...
    if operation == "getReviewReadiness" {
        ...
        return Ok(envelope(
            id,
            review_readiness_status(&blockers),
