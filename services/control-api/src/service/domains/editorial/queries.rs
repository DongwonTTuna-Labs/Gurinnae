...
pub(super) async fn get_publication_preview(
    ...
    .map_err(db)?
    .ok_or(ServiceError::NotFound)?;
    Ok(json!({"previewHash":row.preview_sha256.trim(),
        "caseId":case_id,"snapshotId":snapshot,
        "publicPayload":row.preview_payload,
        "validation":[],"renderedRoutes":[],
        "expiresAt":format_time(row.expires_at)?}))
}
...
pub(super) async fn get_publish_confirmation(
    ...
    .map_err(db)?
    .ok_or(ServiceError::NotFound)?;
    let current = row.current_review_snapshot_id;
    let approved = row.approved.ok_or_else(unexpected_null)?;
    let kill_switch = row.kill_switch.ok_or_else(unexpected_null)?;
    let unresolved = row.unresolved_blockers;
    ...
    Ok(envelope(
        ...
        json!({"caseId":case_id,"reviewSnapshotId":snapshot,
            ...
            "snapshotHash":row.snapshot_sha256.trim(),
...
pub(super) async fn get_review_snapshot(
    ...
    .map_err(db)?
    .ok_or(ServiceError::NotFound)?;
    let payload = row.snapshot_payload;
    Ok(json!({"snapshotId":id,"caseId":row.case_id,
        ...
        "automatedGates":row.automated_gate_results,
        "independence":{},"unresolvedBlockers":row.unresolved_blockers,
        "diffFromCurrent":[]}))
}
...
pub(super) async fn list_case_corrections(
    ...
    let case_id = query_uuid(parameters, "caseId")?;
    let items = sqlx::query_scalar!("SELECT COALESCE(jsonb_agg(jsonb_build_object('id',id,'caseId',case_id,'sourceRevision',source_revision,'targetRevision',target_revision,'summary',summary,'reason',reason,'status',status,'resolution',resolution,'version',version) ORDER BY created_at DESC),'[]'::jsonb) FROM editorial.corrections WHERE case_id=$1", case_id).fetch_one(pool).await.map_err(db)?.ok_or_else(unexpected_null)?;
...
pub(super) async fn list_case_responses(
    ...
    let case_id = query_uuid(parameters, "caseId")?;
    let items = sqlx::query_scalar!("SELECT COALESCE(jsonb_agg(jsonb_build_object('id',id,'caseId',case_id,'responseRequestId',response_request_id,'partyName',party_name,'submittedAt',submitted_at,'verifiedAt',verified_at,'publicExcerpt',public_excerpt,'publicationConsent',publication_consent,'editorialStatus',editorial_status,'version',version) ORDER BY submitted_at DESC),'[]'::jsonb) FROM editorial.responses WHERE case_id=$1", case_id).fetch_one(pool).await.map_err(db)?.ok_or_else(unexpected_null)?;
    list_response(items, parameters)
}
...
pub(super) async fn get_communication_delivery_receipt(
    ...
    let value: Option<Value> =
        sqlx::query_scalar!("SELECT ops.read_communication_delivery_receipt_v1($1)", id)
            .fetch_optional(pool)
            .await
            .map_err(db)?
...
pub(super) async fn get_response_appeal_workspace(
    ...
    let value: Option<Value> = sqlx::query_scalar!("SELECT ops.read_appeal_workspace_v1($1)", id)
        .fetch_optional(pool)
        .await
        .map_err(db)?
        .map(|value| value.ok_or_else(unexpected_null))
