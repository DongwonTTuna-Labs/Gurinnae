...
verify-specs:
	...
	docker run --rm --network none --user "$$(id -u):$$(id -g)" --env HOME=/tmp --env PYTHONDONTWRITEBYTECODE=1 --volume "$(CURDIR):/workspace" --workdir /workspace gurine-authority-validator:13.0.0 python -B scripts/validate_final_spec.py --strict
...
verify-prearchive: verify-specs verify-codegen test-rust-workspace test-sqlx-prepare verify-bun verify-runtime verify-containers test-ui-e2e test-ui-visual
	@printf 'all specification, code generation, source, runtime, container, recovery and UI prearchive gates: PASS\n'
...
clean-extraction-verify:
	$(PYTHON_ENV) $(PYTHON) -B scripts/verify_source_archive.py --write-receipt
