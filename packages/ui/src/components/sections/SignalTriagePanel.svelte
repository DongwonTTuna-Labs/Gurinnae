...
const branches = [
  ...
  ["추가 자료 필요", "NEEDS_DATA", "확인할 자료와 담당자를 지정한 뒤 보류합니다."],
  ["중복 신호로 표시", "MARK_DUPLICATE", "원본 신호를 지정하고 중복 관계를 보존합니다."],
  ["사건으로 승격", "PROMOTE_TO_CASE", "사건 질문·담당자·기한을 함께 생성합니다."],
  ["기존 사건 연결", "LINK_TO_CASE", "이미 소유된 사건으로 연결하고 수신을 확인합니다."],
] as const;
const triageRecord = $derived(firstRecord(runtime.data));
const duplicateTarget = $derived(value(triageRecord, ["duplicateTarget", "targetSignalId", "relatedSignal"]));
const relationship = $derived(value(triageRecord, ["relationship", "duplicateRelationship", "relation"]));
function firstRecord(data: Record<string, unknown>): Record<string, unknown> | null {
  for (const item of Object.values(data)) if (isRecord(item)) return item;
  return null;
}
function isRecord(value: unknown): value is Record<string, unknown> { return typeof value === "object" && value !== null && !Array.isArray(value); }
function value(record: Record<string, unknown> | null, keys: readonly string[]): string {
  if (!record) return "확인 필요";
...
<OperationData {runtime} mode="table" emptyLabel="현재 분류할 신호가 없습니다." />
