...
const metrics = $derived(
  projection?.fields
    .filter(
      (field) =>
        field.known && field.value !== null && field.name.startsWith("metric."),
    )
    .reduce((groups, field) => {
...
const funnel = $derived(
  projection?.fields.filter(
    (field) =>
      field.known && field.value !== null && field.name.startsWith("funnel."),
  ) ?? [],
);
  ...
  {#if projection}
    ...
    <dl class="business-health-facts">
      {#each facts as field (field.name)}
        <div class="business-health-fact"><dt>{field.label}</dt><dd><ProjectionValue name={field.name} label={field.label} value={field.value} /></dd></div>
      {/each}
    </dl>
    {#if funnel.length > 0}
      <section aria-labelledby="business-health-funnel-heading">
        ...
        <ul class="business-health-funnel">
          {#each funnel as field (field.name)}<li><span>{field.label}</span><div class="business-health-value"><ProjectionValue name={field.name} label={field.label} value={field.value} /></div></li>{/each}
        </ul>
      </section>
    {/if}
    {#if metrics.size > 0}
      <section aria-labelledby="business-health-metrics-heading">
        ...
        {#each [...metrics.entries()] as [metricId, metricFields] (metricId)}
          <details class="business-health-metric" open>
            <summary>{metricFields[0]?.label ?? "지표"}</summary>
            <dl>{#each metricFields as field (field.name)}<div><dt>{field.label}</dt><dd><ProjectionValue name={field.name} label={field.label} value={field.value} /></dd></div>{/each}</dl>
          </details>
        {/each}
      </section>
    {/if}
  {:else}
    <p class="inline-state" role="status">사업 상태 정보를 불러오는 중입니다.</p>
  {/if}
</div>
