...
<div class="omnichannel-approval" data-testid="omnichannel-approval" data-state={projection?.state ?? "UNKNOWN"} aria-busy={runtime.state === "loading"}>
  <p class="approval-safety">외부 채널 전송은 사람의 명시적 승인과 정확한 전송 내용·수신자·동의·영수증이 모두 확인될 때만 열립니다. 승인 전에는 전송하지 않습니다.</p>
  ...
  {/if}
  {#if projection}<OperationData {runtime} {projection} mode="cards" emptyLabel="승인 가능한 제안이 없습니다." />{:else}<p role="status">승인 정보를 불러오는 중입니다.</p>{/if}
  {#if selectedTarget && !canDecide}<p class="inline-state conflict" role="status">제안 버전 또는 승인 요약값이 최신이 아니어서 결정을 기록할 수 없습니다.</p>{/if}
  {#if canDecide}
  ...
  {/if}
  {#if runtime.state === "receipt"}<p class="inline-state" role="status">전달 영수증이 저장되었습니다. 같은 내용은 영수증 확인 전 재전송하지 않습니다.</p>
  {:else if runtime.state === "conflict"}<p class="inline-state conflict" role="status">승인 대상 버전이 바뀌었습니다. 새 미리보기를 확인해야 합니다.</p>
  {:else if runtime.state === "stale"}<p class="inline-state stale" role="status">승인 정보가 오래되었습니다. 새 요약값과 버전을 다시 확인하세요.</p>{/if}
</div>
