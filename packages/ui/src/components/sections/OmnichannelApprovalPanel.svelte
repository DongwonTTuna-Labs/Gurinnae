...
<div class="omnichannel-approval" data-testid="omnichannel-approval" data-state={projection?.state ?? "UNKNOWN"} aria-busy={runtime.state === "loading"}>
  ...
  {#if selectedTarget && !canDecide}<p class="inline-state conflict" role="alert">제안 버전 또는 승인 지문이 최신이 아니어서 결정을 기록할 수 없습니다.</p>{/if}
  {#if canDecide}
  ...
  {:else if runtime.state === "conflict"}<p class="inline-state conflict" role="alert">승인 대상 버전이 바뀌었습니다. 새 preview를 확인해야 합니다.</p>
