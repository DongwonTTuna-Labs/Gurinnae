...
function fact(label: string, value: unknown, extra: Partial<Int002Fact> = {}): Int002Fact | null {
  const text = string(value) ?? (typeof value === "number" || typeof value === "boolean" ? String(value) : null);
...
function projection(
  ...
  return { summary, status, facts: facts.filter((item): item is Int002Fact => item !== null), asOf, provenance };
}

/** Maps only the fields authorised by Int002ScreenVmV1. Unknown DTO fields are ignored. */
export function toInt002ViewModel(data: Record<string, unknown>): Int002ViewModel {
  ...
  const taskFacts = taskItems.flatMap((item, index) => {
    ...
    return [
      ...
      fact("상태", row.status),
      fact("기한", row.dueAt),
      fact("대상", row.objectType && row.objectId ? `${String(row.objectType)} · ${String(row.objectId)}` : null),
    ];
  });
  ...
  const selectedDigestCurrent = selectedApprovalDigest !== null && /^[0-9a-f]{64}$/i.test(selectedApprovalDigest);
  const selectedTarget = selectedProposalId && selectedProposalVersion !== null && selectedApprovalDigest
    ? {
        ...
        expectedAssignmentVersion: number(selected?.expectedAssignmentVersion),
        expectedApprovalDigest: selectedApprovalDigest,
        digestCurrent: selectedDigestCurrent,
        loadState: selectedDigestCurrent ? ("READY" as const) : ("BLOCKED" as const),
      }
    : null;
  ...
  return {
    ...
    taskFilters: {
      overdueOnly: typeof filters.overdueOnly === "boolean" ? filters.overdueOnly : null,
      taskStatus,
      taskType,
    },
    handoffState: projection(
      ...
      [fact("대기 중인 승인", approvalItems.length), fact("기준 시각", queueAsOf)],
      queueAsOf,
    ...
    handoffUnknown: projection(
      ...
      [fact("상세 제안", approvalItems.length > 0 ? "승인 전 상세 payload 확인 필요" : null)],
      queueAsOf,
