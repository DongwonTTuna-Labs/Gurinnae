...
어떤 개인정보를 왜 얼마나 보관하고 누구와 공유하는지 확인한다.

## 이 화면이 즉시 답해야 하는 질문

- 어떤 정보가 수집되는가?
- 소명·정정·구독 데이터는 어떻게 다른가?
- 권리를 어떻게 행사하는가?
...
|---:|---|---|---|---|
| 1 | `controller` | 처리자 | `StructuredContentSection` | 개인정보 처리 주체와 연락처. |
| 2 | `categories` | 정보 범주 | `StructuredContentSection` | surface별. |
| 3 | `purposes` | 목적·법적 근거 | `EvidenceLedger` | 명확한 매핑. |
| 4 | `retention` | 보존 | `StructuredContentSection` | record type별. |
| 5 | `processors` | 처리위탁·국외 | `StructuredContentSection` | provider. |
| 6 | `rights` | 권리 | `StructuredContentSection` | 요청 방법. |
| 7 | `security` | 보호 | `StructuredContentSection` | 고수준. |
| 8 | `history` | 변경 이력 | `RevisionTimeline` | effective date. |
...
|---|---|---|---|---|---|
| `privacy-contact` | 개인정보 문의 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `view-history` | 이전 버전 | `NAVIGATION` | `NONE` | `none` | `local-only` |

Primary action: `privacy-contact`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `getPrivacyPolicy` | `public-api` | `GET` | `/v1/content/privacy` | `getPrivacyPolicyQuery` | `PrivacyPolicyResponse` |
...
- `public.privacy.viewed`

본문·magic token·scoped session·개인정보·첨부파일명·근거 원문은 analytics payload에 포함하지 않는다.

## 금지

- 실제 controller 미정 상태로 출시
- 모호한 무기한 보존

## Implementation files

- Page: `apps/public-web/src/routes/privacy/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/privacy/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-031.ts`
- E2E: `tests/e2e/screens/pub-031.spec.ts`
- Visual: `tests/visual/pub-031.spec.ts`
