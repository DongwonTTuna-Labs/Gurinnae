...
질문별 답변·참고·공개 동의를 임시 저장하며 작성한다.

## 이 화면이 즉시 답해야 하는 질문

- 내 답변이 저장됐는가?
- 질문별로 무엇을 작성해야 하는가?
- 공개 가능한 부분을 구분할 수 있는가?
...
|---:|---|---|---|---|
| 1 | `progress` | 진행 상태 | `StatusAndRevisionHeader` | 단계와 저장. |
| 2 | `questions` | 질문별 답변 | `GuidedFormSection` | answer·reference. |
| 3 | `statement` | 전체 입장 | `StructuredContentSection` | 선택적. |
| 4 | `consent` | 공개 동의 | `StructuredContentSection` | FULL/REDACTED/SUMMARY/NO_CONSENT. |
| 5 | `save` | 저장 상태 | `StatusAndRevisionHeader` | last saved·conflict. |
...
|---|---|---|---|---|---|
| `save-draft` | 답변 임시 저장 | `COMMAND` | `SCOPED_TOKEN` | `none` | `saveResponseDraft` |
| `next` | 첨부자료로 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `discard-change` | 변경 취소 | `COMMAND` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `getResponseDraft` | `submission-api` | `GET` | `/v1/response-session/draft` | `getResponseDraftQuery` | `ResponseDraftResponse` |
| `saveResponseDraft` | `submission-api` | `PUT` | `/v1/response-session/draft` | `saveResponseDraftRequest` | `saveResponseDraftReceipt` |
...
- `response.answer.viewed`
- `response.draft.saved`
- `response.answer.validation_failed`
...
- Page: `apps/response-portal/src/routes/respond/answer/+page.svelte`
- Server load/actions: `apps/response-portal/src/routes/respond/answer/+page.server.ts`
- Layout: `apps/response-portal/src/routes/+layout.server.ts`
- Error boundary: `apps/response-portal/src/routes/+error.svelte`
- View model: `apps/response-portal/src/lib/view-models/rsp-003.ts`
- E2E: `tests/e2e/screens/rsp-003.spec.ts`
- Visual: `tests/visual/rsp-003.spec.ts`
