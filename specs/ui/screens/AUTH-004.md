...
Test prefix: `auth_004`

## 사용자 목적

세션 만료가 작업에 미친 영향과 draft 보존 상태를 이해하고 다시 로그인한다.

## 이 화면이 즉시 답해야 하는 질문

- 내 입력은 보존됐는가?
- 다시 로그인하면 원래 위치로 돌아가는가?
...
|---|---|---|---|---|---|
| `sign-in-again` | 다시 로그인 | `COMMAND` | `ANONYMOUS_PROOF` | `none` | `startOidcLogin` |
| `discard-local` | 로컬 임시 상태 삭제 | `COMMAND` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `startOidcLogin` | `identity-provider` | `GET` | `/auth/login` | `startOidcLoginQuery` | `startOidcLoginReceipt` |
...
- `internal.auth.session_expired_viewed`
- `internal.auth.signin_started`

본문·magic token·scoped session·개인정보·첨부파일명·근거 원문은 analytics payload에 포함하지 않는다.

## 금지

- 저장되지 않은 작업을 무조건 보존했다고 표현

## Implementation files

- Page: `apps/review-console/src/routes/auth/session-expired/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/auth/session-expired/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/auth-004.ts`
- E2E: `tests/e2e/screens/auth-004.spec.ts`
- Visual: `tests/visual/auth-004.spec.ts`
