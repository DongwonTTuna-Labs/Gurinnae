...
Test prefix: `auth_002`

## 사용자 목적

로그인 또는 고위험 행동에 필요한 추가 인증을 완료한다.
...
|---:|---|---|---|---|
| 1 | `reason` | 재인증 이유 | `StructuredContentSection` | 대상 action·만료. |
| 2 | `methods` | 인증 방법 | `StructuredContentSection` | WebAuthn/MFA. |
| 3 | `recovery` | 복구 | `StructuredContentSection` | 지원·fallback. |
...
|---|---|---|---|---|---|
| `verify` | 추가 인증 시작 | `COMMAND` | `ACTIVE_SESSION` | `none` | `startStepUpAuthentication` |
| `cancel` | 작업으로 돌아가기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `startStepUpAuthentication` | `identity-provider` | `POST` | `/auth/step-up/start` | `startStepUpAuthenticationRequest` | `startStepUpAuthenticationReceipt` |
...
- `internal.auth.step_up_started`
- `internal.auth.step_up_completed`
...
- Page: `apps/review-console/src/routes/auth/verify/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/auth/verify/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/auth-002.ts`
- E2E: `tests/e2e/screens/auth-002.spec.ts`
- Visual: `tests/visual/auth-002.spec.ts`
