...
공개 기록의 정정·철회·중요 변경을 투명하게 탐색한다.

## 이 화면이 즉시 답해야 하는 질문

- 최근 무엇이 바뀌었는가?
- 왜 바뀌었고 어떤 페이지에 영향을 줬는가?
...
|---:|---|---|---|---|
| 1 | `principle` | 정정 정책 | `RevisionAndCorrectionPanel` | 오류를 숨기지 않는 원칙. |
| 2 | `filters` | 필터 | `UnifiedSearch` | 유형·기간·영향. |
| 3 | `records` | 정정 목록 | `RevisionAndCorrectionPanel` | 대상·before/after·reason·date. |
| 4 | `feed` | 구독·정정 알림 | `RevisionAndCorrectionPanel` | 정정 알림. |
...
|---|---|---|---|---|---|
| `open-correction` | 정정 상세 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `subscribe` | 정정 알림 구독 | `COMMAND` | `ANONYMOUS_PROOF` | `none` | `createSubscription` |

Primary action: `open-correction`
...
- `public.correction_list.viewed`
- `public.correction.opened`
...
- Page: `apps/public-web/src/routes/corrections/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/corrections/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-018.ts`
- E2E: `tests/e2e/screens/pub-018.spec.ts`
- Visual: `tests/visual/pub-018.spec.ts`
