...
사건·기관·업체·정정 업데이트의 범위와 빈도를 선택해 이메일을 검증한다.
...
|---:|---|---|---|---|
| 1 | `topic` | 구독 대상 | `RevisionAndCorrectionPanel` | case/agency/supplier/corrections. |
| 2 | `events` | 알림 유형 | `KnownUnknownResponse` | 공개·소명·정정·공식확인. |
| 3 | `frequency` | 빈도 | `StructuredContentSection` | 즉시/일간/주간. |
| 4 | `email` | 이메일 | `AgentSuggestionPanel` | verification. |
| 5 | `privacy` | 개인정보 | `StructuredContentSection` | retention·해지. |
...
|---|---|---|---|---|---|
| `request-verification` | 검증 이메일 보내기 | `COMMAND` | `ANONYMOUS_PROOF` | `none` | `createSubscription` |
...
| `verifySubscription` | `submission-api` | `POST` | `/v1/submission-session/subscription:verify` | `verifySubscriptionRequest` | `verifySubscriptionResult` |
...
- `public.subscription.verification_requested`
...
- Page: `apps/public-web/src/routes/subscribe/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/subscribe/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-029.ts`
- E2E: `tests/e2e/screens/pub-029.spec.ts`
- Visual: `tests/visual/pub-029.spec.ts`
