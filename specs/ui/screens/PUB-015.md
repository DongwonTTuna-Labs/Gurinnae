...
구린네가 포함·미포함하는 source, 기간, 기관, 지연과 누락을 이해한다.
...
|---:|---|---|---|---|
| 1 | `summary` | coverage 요약 | `StatusAndRevisionHeader` | 전체 범위와 current status. |
| 2 | `sources` | source별 범위 | `CoverageStatement` | 기간·기관·갱신. |
| 3 | `gaps` | known gaps | `StructuredContentSection` | 미수집·누락·license. |
| 4 | `latency` | 처리 지연 | `StructuredContentSection` | source→publication latency. |
| 5 | `changes` | 범위 변경 | `CoverageStatement` | effective history. |
...
|---|---|---|---|---|---|
| `open-source` | source 상세 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `download-coverage` | coverage JSON 다운로드 | `DOWNLOAD` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `getCoverage` | `public-api` | `GET` | `/v1/coverage` | `getCoverageQuery` | `CoverageResponse` |
...
- `public.coverage.viewed`
- `public.coverage.source_opened`
...
- Page: `apps/public-web/src/routes/coverage/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/coverage/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-015.ts`
- E2E: `tests/e2e/screens/pub-015.spec.ts`
- Visual: `tests/visual/pub-015.spec.ts`
