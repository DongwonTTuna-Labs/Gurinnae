...
모든 답변·첨부·동의·권한을 검토하고 명시적으로 제출한다.
...
|---:|---|---|---|---|
| 1 | `answers` | 답변 확인 | `KnownUnknownResponse` | 질문별 summary와 change link. |
| 2 | `attachments` | 첨부 확인 | `KnownUnknownResponse` | scan·분류. |
| 3 | `consent` | 공개 동의 확인 | `KnownUnknownResponse` | 정확한 선택. |
| 4 | `authority` | 권한 확인 | `KnownUnknownResponse` | 조직을 대신해 제출할 권한. |
| 5 | `consequence` | 제출 후 | `GuidedFormSection` | immutable submission·추가자료 방식. |
...
|---|---|---|---|---|---|
| `change-answer` | 답변 수정 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `change-files` | 첨부 수정 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `submit` | 답변 제출 | `FORM_SUBMIT` | `SCOPED_TOKEN` | `none` | `submitResponse` |

Primary action: `change-answer`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `getResponseSubmissionPreview` | `submission-api` | `GET` | `/v1/response-session/preview` | `getResponseSubmissionPreviewQuery` | `ResponseSubmissionPreviewResponse` |
| `submitResponse` | `submission-api` | `POST` | `/v1/response-session:submit` | `submitResponseRequest` | `submitResponseReceipt` |
...
- `response.review.viewed`
- `response.submission.attempted`
- `response.submission.completed`
...
- Page: `apps/response-portal/src/routes/respond/review/+page.svelte`
- Server load/actions: `apps/response-portal/src/routes/respond/review/+page.server.ts`
- Layout: `apps/response-portal/src/routes/+layout.server.ts`
- Error boundary: `apps/response-portal/src/routes/+error.svelte`
- View model: `apps/response-portal/src/lib/view-models/rsp-005.ts`
- E2E: `tests/e2e/screens/rsp-005.spec.ts`
- Visual: `tests/visual/rsp-005.spec.ts`
