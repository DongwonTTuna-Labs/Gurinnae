...
|---:|---|---|---|---|
| 1 | `mission` | 운영 목적 | `StructuredContentSection` | 구린네의 운영 목적과 공개 범위. |
| 2 | `non-goals` | 하지 않는 일 | `StructuredContentSection` | 판정·게시판·ranking 금지. |
| 3 | `team` | 운영 주체 | `OperationsStatusPanel` | 법인·책임자·이해상충. |
| 4 | `process` | 운영 방식 | `RevisionAndCorrectionPanel` | 조사·review·정정. |
| 5 | `contact` | 연락 | `StructuredContentSection` | 문의 유형. |
...
|---|---|---|---|---|---|
| `view-governance` | 운영 원칙 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `contact` | 문의 | `NAVIGATION` | `NONE` | `none` | `local-only` |

Primary action: `view-governance`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `getAboutContent` | `public-api` | `GET` | `/v1/content/about` | `getAboutContentQuery` | `AboutContentResponse` |
...
- Page: `apps/public-web/src/routes/about/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/about/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-022.ts`
- E2E: `tests/e2e/screens/pub-022.spec.ts`
- Visual: `tests/visual/pub-022.spec.ts`
