...
Test prefix: `rule_001`

## 사용자 목적

규칙의 active/shadow/retired version, owner, 평가, 적용 source를 탐색한다.

## 이 화면이 즉시 답해야 하는 질문

- 어떤 규칙과 version이 운영 중인가?
- 평가와 최근 변경은 무엇인가?
- 어떤 source와 사건에 영향을 주는가?
...
|---:|---|---|---|---|
| 1 | `status` | 규칙 상태 | `StatusAndRevisionHeader` | active/shadow/retired. |
| 2 | `filters` | 필터 | `UnifiedSearch` | type/owner/state. |
| 3 | `registry` | 규칙 목록 | `DataCollection` | version·eval·last run. |
| 4 | `changes` | 최근 변경 | `StructuredContentSection` | activation/rollback. |
...
|---|---|---|---|---|---|
| `listInternalRules` | `control-api` | `GET` | `/v1/internal/queries/list-internal-rules` | `listInternalRulesQuery` | `InternalRulesPage` |
...
- `internal.rule_registry.viewed`
- `internal.rule.opened`

본문·magic token·scoped session·개인정보·첨부파일명·근거 원문은 analytics payload에 포함하지 않는다.

## 금지

- threshold만 있는 opaque rule
- public corruption score

## Implementation files

- Page: `apps/review-console/src/routes/internal/rules/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/rules/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/rule-001.ts`
- E2E: `tests/e2e/screens/rule-001.spec.ts`
- Visual: `tests/visual/rule-001.spec.ts`
