...
모든 최신 gate와 영향 범위를 재검증하고 publication을 게시한 뒤 receipt를 확인한다.

## 이 화면이 즉시 답해야 하는 질문

- 모든 승인이 최신인가?
- 어떤 제목·상태·revision이 공개되는가?
- 게시 후 어떤 projection·알림이 발생하는가?
...
|---:|---|---|---|---|
| 1 | `target` | 게시 대상 | `StructuredContentSection` | case·snapshot·slug·state. |
| 2 | `gates` | 최종 gate | `KnownUnknownResponse` | review·response·privacy·source. |
| 3 | `impact` | 영향 | `StructuredContentSection` | public route·notifications·cache. |
| 4 | `reauth` | 재인증 | `StructuredContentSection` | assurance. |
| 5 | `receipt` | 게시 결과 | `StatusAndRevisionHeader` | publication ID·projection status. |
...
|---|---|---|---|---|---|
| `publish` | 최신 snapshot 게시 | `DESTRUCTIVE_CONFIRMATION` | `STEP_UP` | `publication.publish` | `publishCase` |
| `cancel` | 검토로 돌아가기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `open-public` | 공개 페이지 열기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `getPublishConfirmation` | `control-api` | `GET` | `/v1/internal/queries/get-publish-confirmation` | `getPublishConfirmationQuery` | `PublishConfirmationResponse` |
| `publishCase` | `control-api` | `POST` | `/v1/internal/commands/publish-case` | `publishCaseRequest` | `publishCaseReceipt` |
| `getPublicationReceipt` | `control-api` | `GET` | `/v1/internal/queries/get-publication-receipt` | `getPublicationReceiptQuery` | `PublicationReceiptResponse` |
...
- `internal.publication.confirmation_viewed`
- `internal.publication.published`
- `internal.publication.projection_failed`
...
- 일반 confirm dialog
- stale approval
- receipt 없이 success toast만

## Implementation files

- Page: `apps/review-console/src/routes/internal/review/[snapshotId]/publish/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/review/[snapshotId]/publish/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/rev-003.ts`
- E2E: `tests/e2e/screens/rev-003.spec.ts`
- Visual: `tests/visual/rev-003.spec.ts`
