...
|---:|---|---|---|---|
| 1 | `search` | 기관 검색 | `UnifiedSearch` | 공식 명칭과 과거 명칭. |
| 2 | `filters` | 지역·유형 | `UnifiedSearch` | coverage-aware filter. |
| 3 | `results` | 기관 목록 | `CoverageStatement` | 기관 identity·관할·coverage. |
| 4 | `coverage` | 수집 범위 | `CoverageStatement` | 미포함 기관 설명. |
...
|---|---|---|---|---|---|
| `open-agency` | 기관 보기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `apply-filter` | 필터 적용 | `COMMAND` | `NONE` | `none` | `local-only` |

Primary action: `open-agency`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `listAgencies` | `public-api` | `GET` | `/v1/agencies` | `listAgenciesQuery` | `AgenciesPage` |
...
- `public.agency_list.viewed`
- `public.agency.opened`
...
- Page: `apps/public-web/src/routes/agencies/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/agencies/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-007.ts`
- E2E: `tests/e2e/screens/pub-007.spec.ts`
- Visual: `tests/visual/pub-007.spec.ts`
