...
source 실행을 상태·기간·checkpoint·record count·오류로 탐색한다.
...
|---:|---|---|---|---|
| 1 | `summary` | run health | `OperationsStatusPanel` | success/failure/lag. |
| 2 | `filters` | 필터 | `UnifiedSearch` | status/time/trigger. |
| 3 | `runs` | run 목록 | `DataCollection` | records·checkpoint·duration·error. |
| 4 | `schedule` | schedule | `StructuredContentSection` | next run. |
...
| `retry-run` | 안전한 재시도 | `COMMAND` | `ACTIVE_SESSION` | `sources.operate` | `retrySourceRun` |

Primary action: `retry-run`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `listSourceRuns` | `control-api` | `GET` | `/v1/internal/queries/list-source-runs` | `listSourceRunsQuery` | `SourceRunsPage` |
...
- `internal.source_runs.viewed`
- `internal.source_run.opened`
- `internal.source_run.retried`
...
- 중복 방지 없이 retry
- secret/error payload 원문 노출

## Implementation files

- Page: `apps/review-console/src/routes/internal/sources/[sourceId]/runs/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/sources/[sourceId]/runs/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/src-003.ts`
- E2E: `tests/e2e/screens/src-003.spec.ts`
- Visual: `tests/visual/src-003.spec.ts`
