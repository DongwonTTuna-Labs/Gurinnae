...
조직 사용자의 identity·상태·role·pending grant·offboarding을 관리한다.

## 이 화면이 즉시 답해야 하는 질문

- 누가 활성 사용자이며 어떤 권한을 갖는가?
- 만료·충돌·offboarding 대상은 누구인가?
...
|---:|---|---|---|---|
| 1 | `summary` | 접근 상태 | `StatusAndRevisionHeader` | active/disabled/pending. |
| 2 | `filters` | 필터 | `UnifiedSearch` | role/status/team. |
| 3 | `users` | 사용자 목록 | `DataCollection` | identity·roles·last access. |
| 4 | `requests` | 접근 요청 | `StructuredContentSection` | pending. |
| 5 | `review` | 정기 검토 | `DecisionReviewPanel` | due grants. |
...
|---|---|---|---|---|---|
| `open-user` | 사용자 보기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `review-access` | 접근 검토 시작 | `COMMAND` | `RECENT_SESSION` | `roles.grant` | `startAccessReview` |
| `invite` | 사용자 초대 | `COMMAND` | `ACTIVE_SESSION` | `users.manage` | `inviteUser` |

Primary action: `review-access`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `listUsers` | `control-api` | `GET` | `/v1/internal/queries/list-users` | `listUsersQuery` | `UsersPage` |
| `startAccessReview` | `control-api` | `POST` | `/v1/internal/commands/start-access-review` | `startAccessReviewRequest` | `startAccessReviewReceipt` |
| `inviteUser` | `control-api` | `POST` | `/v1/internal/commands/invite-user` | `inviteUserRequest` | `inviteUserReceipt` |
...
- `internal.users.viewed`
- `internal.user.opened`
- `internal.access_review.started`
...
- Page: `apps/review-console/src/routes/internal/admin/users/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/admin/users/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/adm-001.ts`
- E2E: `tests/e2e/screens/adm-001.spec.ts`
- Visual: `tests/visual/adm-001.spec.ts`
