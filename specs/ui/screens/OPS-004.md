...
모델·OCR·storage·egress 비용을 예산 envelope와 workload 가치에 연결해 관리한다.

## 이 화면이 즉시 답해야 하는 질문

- 현재 spend와 forecast는 얼마인가?
- 어떤 작업이 예산을 소모하는가?
- 한도 초과 시 무엇이 중단되는가?
...
|---:|---|---|---|---|
| 1 | `envelope` | 예산 envelope | `AgentSuggestionPanel` | daily/monthly. |
| 2 | `business-health` | Business Health | `StructuredContentSection` | qualification·activation·retention·revenue·margin·SLA의 현재 projection. |
| 3 | `spend` | 사용량 | `StructuredContentSection` | provider/workload/case bucket. |
| 4 | `forecast` | forecast | `StructuredContentSection` | confidence/assumption. |
| 5 | `limits` | 한도 | `StructuredContentSection` | soft/hard/fallback. |
| 6 | `alerts` | 경고 | `StructuredContentSection` | threshold. |
| 7 | `changes` | 변경 이력 | `RevisionTimeline` | approver/reason. |
...
|---|---|---|---|---|---|
| `change-limit` | 예산 한도 변경 | `DESTRUCTIVE_CONFIRMATION` | `STEP_UP` | `budgets.manage` | `updateBudgetLimit` |
| `export` | 비용 보고서 | `DOWNLOAD` | `NONE` | `audit.read` | `local-only` |
| `open-provider` | 공급자 보기 | `NAVIGATION` | `NONE` | `none` | `local-only` |

Primary action: `change-limit`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `getBudgetOverview` | `control-api` | `GET` | `/v1/internal/queries/get-budget-overview` | `getBudgetOverviewQuery` | `BudgetOverviewResponse` |
| `updateBudgetLimit` | `control-api` | `POST` | `/v1/internal/commands/update-budget-limit` | `updateBudgetLimitRequest` | `updateBudgetLimitReceipt` |
| `exportCostReport` | `control-api` | `GET` | `/v1/internal/queries/export-cost-report` | `exportCostReportQuery` | `BinaryDownload` |
...
- `internal.budgets.viewed`
- `internal.budget_limit.updated`
- `internal.cost_report.exported`

본문·magic token·scoped session·개인정보·첨부파일명·근거 원문은 analytics payload에 포함하지 않는다.

## 금지

- 개별 조사자 비용 leaderboard
- 실제 API key/billing credential

## Implementation files

- Page: `apps/review-console/src/routes/internal/operations/budgets/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/operations/budgets/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/ops-004.ts`
- E2E: `tests/e2e/screens/ops-004.spec.ts`
- Visual: `tests/visual/ops-004.spec.ts`
