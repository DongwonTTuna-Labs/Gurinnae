...
공개 정보·API·다운로드 사용 조건과 책임 한계를 이해한다.

## 이 화면이 즉시 답해야 하는 질문

- 자료를 어떻게 사용할 수 있는가?
- source별 권리는 무엇인가?
- 정정된 데이터를 어떻게 다뤄야 하는가?
...
|---:|---|---|---|---|
| 1 | `service` | 서비스 조건 | `AgentSuggestionPanel` | 서비스 제공 범위와 이용 조건. |
| 2 | `content` | 콘텐츠 | `StatusAndRevisionHeader` | attribution·revision. |
| 3 | `data` | 데이터·공개 인터페이스 | `StructuredContentSection` | license·rate. |
| 4 | `prohibited` | 금지 | `StructuredContentSection` | abuse·reidentification. |
| 5 | `liability` | 책임·한계 | `DecisionReviewPanel` | 법률 검토. |
| 6 | `changes` | 변경 | `StructuredContentSection` | notice. |
...
|---|---|---|---|---|---|
| `view-data` | 데이터 안내 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `contact` | 문의 | `NAVIGATION` | `NONE` | `none` | `local-only` |

Primary action: `view-data`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `getTerms` | `public-api` | `GET` | `/v1/content/terms` | `getTermsQuery` | `TermsResponse` |
...
- source 권리를 일괄 소유한다고 주장

## Implementation files

- Page: `apps/public-web/src/routes/terms/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/terms/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-032.ts`
- E2E: `tests/e2e/screens/pub-032.spec.ts`
- Visual: `tests/visual/pub-032.spec.ts`
