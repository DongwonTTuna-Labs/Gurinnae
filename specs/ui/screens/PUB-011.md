...
관측된 공개 계약을 기관·업체·기간·방식·품목으로 탐색한다.
...
|---:|---|---|---|---|
| 1 | `scope` | 계약 관측 범위 | `CoverageStatement` | 사건 공개 여부와 무관한 계약 관측 범위. |
| 2 | `filters` | 계약 필터 | `UnifiedSearch` | 기관·업체·날짜·방식·품목·금액. |
| 3 | `results` | 계약 결과 | `DataCollection` | 계약번호·제목·상태·금액·기관·업체를 포함한 계약 대장. |
| 4 | `limitations` | 비교 한계 | `ComparisonWorkbench` | 금액·단위 정렬 주의. |
...
|---|---|---|---|---|---|
| `open-contract` | 계약 보기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `apply-filter` | 필터 적용 | `COMMAND` | `NONE` | `none` | `local-only` |
| `download` | 결과 다운로드 | `DOWNLOAD` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `listContracts` | `public-api` | `GET` | `/v1/contracts` | `listContractsQuery` | `ContractsPage` |
| `downloadContracts` | `public-api` | `GET` | `/v1/contracts/download` | `downloadContractsQuery` | `BinaryDownload` |
...
- `public.contract_list.viewed`
- `public.contract.opened`
- `public.contracts.downloaded`
...
- Page: `apps/public-web/src/routes/contracts/+page.svelte`
- Server load/actions: `apps/public-web/src/routes/contracts/+page.server.ts`
- Layout: `apps/public-web/src/routes/+layout.server.ts`
- Error boundary: `apps/public-web/src/routes/+error.svelte`
- View model: `apps/public-web/src/lib/view-models/pub-011.ts`
- E2E: `tests/e2e/screens/pub-011.spec.ts`
- Visual: `tests/visual/pub-011.spec.ts`
