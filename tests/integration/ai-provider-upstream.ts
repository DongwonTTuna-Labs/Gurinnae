...
Bun.serve({
  ...
  async fetch(request) {
    ...
    if (
      ...
      return Response.json({ code: "request_redacted_invalid" }, { status: 400 });
    }
    const runId = request.headers.get("x-gurine-ai-agent-run-id");
    const turnId = request.headers.get("x-gurine-ai-provider-turn-id");
    const providerConfigId = request.headers.get("x-gurine-ai-provider-config-id");
    ...
    if (
      ...
      return Response.json({ code: "binding_headers_missing" }, { status: 400 });
    }
    const now = new Date().toISOString();
    const turnSequence = Number(body.turnSequence ?? 1);
    const toolTurn = turnSequence === 1;
    const agentType = request.headers.get("x-gurine-ai-agent-type") ?? "investigator";
    const toolId = ["claim-drafter", "citation-verifier"].includes(agentType)
      ? "evidence.read"
      : "evidence.search";
    const providerRequestIdHash = sha256Hex(`runtime-provider-request:${turnId}`);
    ...
    if (toolTurn) {
      return Response.json({
        toolCall: {
          ...
          toolId,
          request: toolId === "evidence.read" ? { evidence_id: evidenceId } : { query: "runtime" },
        },
        providerReceipt: receipt,
      });
    }
    const citation = evidenceId
      ? [{ evidence_id: evidenceId, locator: evidenceLocator, supports: "검증된 provider egress runtime 근거" }]
      : [];
    return Response.json({
      output: {
        status: "COMPLETED",
