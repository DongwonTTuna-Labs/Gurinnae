...
const concurrencyContractPath = path.join(
  root,
  "specs/application/optimistic-concurrency.runtime.json",
);
const outputArg = process.argv.indexOf("--output");
const outputPath =
  outputArg >= 0 ? path.resolve(process.argv[outputArg + 1]) : null;
...
async function ensureBundledLibraryLinks() {
  ...
  try {
    ...
    throw new Error(
      `embedded PostgreSQL library directory is unavailable: ${error.message}`,
    );
  }
...
async function expectSqlError(name, run, allowedCodes = []) {
  try {
    ...
    if (allowedCodes.length && !allowedCodes.includes(error.code)) {
      throw new Error(
        `${name}: expected SQLSTATE ${allowedCodes.join(", ")}, got ${error.code}: ${error.message}`,
      );
    }
...
async function main() {
  ...
  client = new Client({
    ...
    user: "postgres",
    password,
    database: "gurine_authority_runtime",
  });
  ...
  for (const file of migrationFiles) {
    await client.query(
      await fs.readFile(path.join(migrationDir, file), "utf8"),
    );
  }
  const digest = await migrationDigest(migrationFiles);
  record("clean-migration-apply", {
    migrationCount: migrationFiles.length,
    migrationTreeSha256: digest,
  });

  const version = (
    await client.query("SELECT current_setting('server_version') AS version")
  ).rows[0].version;
  ...
  assert(
    extensionRows.length === 3 &&
      extensionRows.every((row) => row.nspname === "extensions"),
    `extension schema mismatch: ${JSON.stringify(extensionRows)}`,
  );
  ...
  for (const row of securityDefiners) {
    const searchPath = (row.proconfig ?? []).find((value) =>
      value.startsWith("search_path="),
    );
    assert(searchPath, `${row.function_name}: missing fixed search_path`);
    assert(
      !searchPath.includes("public"),
      `${row.function_name}: public is present in search_path`,
    );
    assert(
      searchPath.endsWith("pg_temp"),
      `${row.function_name}: pg_temp is not last in search_path`,
    );
  }
  record("security-definer-search-path", {
    functionCount: securityDefiners.length,
  });
  ...
  assert(
    catalogCounts.tables === 107,
    `expected 107 active tables, got ${catalogCounts.tables}`,
  );
  assert(
    catalogCounts.functions === 72,
    `expected 72 active first-party functions, got ${catalogCounts.functions}`,
  );
  assert(
    catalogCounts.triggers === 37,
    `expected 37 active first-party triggers, got ${catalogCounts.triggers}`,
  );
  assert(
    catalogCounts.policies === 5,
    `expected 5 active RLS policies, got ${catalogCounts.policies}`,
  );
  record("active-catalog-object-counts", catalogCounts);

  const legacyStepUpObjects = (
    await client.query(`
  ...
  assert(
    !legacyStepUpObjects.legacy_table && !legacyStepUpObjects.legacy_function,
    `legacy step-up proof objects remain: ${JSON.stringify(legacyStepUpObjects)}`,
  );
  ...
  const concurrencyContractSha256 = crypto
    .createHash("sha256")
    .update(concurrencyBytes)
    .digest("hex");
  assert(
    concurrencySpec.contractCount === 64 &&
      concurrencySpec.contracts.length === 64,
    "expected 64 optimistic concurrency contracts",
  );
  const catalogColumnRows = (
    await client.query(`
  ...
  for (const contract of concurrencySpec.contracts) {
    ...
    assert(
      guard,
      `${contract.operationId}: guard relation does not resolve: ${contract.guardRelation}`,
    );
    assert(
      mutation,
      `${contract.operationId}: mutation relation does not resolve: ${contract.mutationRelation}`,
    );
    for (const column of contract.guardColumns)
      assert(
        guard.has(column),
        `${contract.operationId}: guard column does not resolve: ${contract.guardRelation}.${column}`,
      );
    for (const column of contract.mutationColumns)
      assert(
        mutation.has(column),
        `${contract.operationId}: mutation column does not resolve: ${contract.mutationRelation}.${column}`,
      );
  }
  record("concurrency-contract-catalog-resolution", {
    contractCount: concurrencySpec.contractCount,
    concurrencyContractSha256,
  });

  const auditSignature =
    "ops.append_audit_event(text,text,text,uuid,text,text,text,text,ops.audit_outcome,text,uuid,jsonb)";
  const auditGrant = (
    await client.query(
      `
  ...
  assert(
    auditGrant.procedure && auditGrant.identity_execute,
    `audit signature/grant mismatch: ${JSON.stringify(auditGrant)}`,
  );
  record("routine-signature-resolution", auditGrant);

  const firstRequestId = (await client.query("SELECT gen_random_uuid() AS id"))
    .rows[0].id;
  const firstAudit = (
    await roleQuery(
      "gurine_control_api",
  ...
  const secondRequestId = (await client.query("SELECT gen_random_uuid() AS id"))
    .rows[0].id;
  const secondAudit = (
    await roleQuery(
      "gurine_control_api",
  ...
  assert(
    chain.first_hash === chain.second_previous_hash,
    "audit previous hash does not link to first event",
  );
  assert(
    chain.second_hash === chain.head_hash &&
      chain.head_event_id === secondAudit &&
      Number(chain.version) === 2,
    "audit head mismatch",
  );
  record("audit-runtime-chain", {
    firstAudit,
    secondAudit,
  ...
  const lifecycleSourceDocumentId = (
    await client.query(
      `
  ...
  const parsedSource = (
    await client.query(
      "SELECT status::text, parser_name, parser_version FROM raw.source_documents WHERE id=$1",
      [lifecycleSourceDocumentId],
    )
  ).rows[0];
  assert(
    parsedSource.status === "PARSED" &&
      parsedSource.parser_name === "runtime-parser",
    "valid source lifecycle transition failed",
  );
  record("source-document-valid-lifecycle-transition");
  await expectSqlError(
    "source-document-invalid-lifecycle-rejected",
    () =>
      roleQuery(
        "gurine_ingest_worker",
  ...
  const terminalRuleRun = (
    await client.query(
      "SELECT status, record_count, signal_count FROM core.rule_runs WHERE id=$1",
      [ruleRunId],
    )
  ).rows[0];
  assert(
    terminalRuleRun.status === "SUCCEEDED" &&
      Number(terminalRuleRun.record_count) === 10,
    "valid rule-run terminal transition failed",
  );
  record("rule-run-valid-terminal-transition");
  await expectSqlError(
    "rule-run-terminal-mutation-rejected",
    () =>
      roleQuery(
        "gurine_analysis_worker",
  ...
  const immutableCaseId = (
    await client.query(
      "INSERT INTO editorial.cases(title) VALUES('Immutable review canary') RETURNING id",
    )
  ).rows[0].id;
  const reviewSnapshotId = (
    await client.query(
      `
    ...
    RETURNING id
  `,
      [immutableCaseId, "8".repeat(64), "11111111-1111-4111-8111-111111111111"],
    )
  ).rows[0].id;
  await expectSqlError(
    "review-snapshot-mutation-rejected",
    () =>
      roleQuery(
        "gurine_control_api",
  ...
  await expectSqlError(
    "public-role-private-schema-denied",
    () =>
      roleQuery("gurine_public_api", "SELECT 1 FROM editorial.cases LIMIT 1"),
    ["42501"],
  );
  await expectSqlError(
    "public-role-write-denied",
    () =>
      roleQuery(
        "gurine_public_api",
        "INSERT INTO public.cases(id,slug,title,public_state,latest_revision,summary,published_at,updated_at,source_freshness) VALUES(gen_random_uuid(),'forbidden','x','x',1,'x',now(),now(),'{}')",
      ),
    ["42501"],
  );
  await expectSqlError(
    "identity-role-editorial-denied",
    () =>
      roleQuery("gurine_identity_api", "SELECT 1 FROM editorial.cases LIMIT 1"),
    ["42501"],
  );
  await expectSqlError(
    "submission-direct-write-denied",
    () =>
      roleQuery(
        "gurine_submission_api",
        "INSERT INTO intake.response_submissions(response_request_id,draft_version,submission_sha256,answers_encrypted,publication_consent,receipt_token_hash) VALUES(gen_random_uuid(),1,repeat('a',64),decode('00','hex'),'{}',repeat('b',64))",
      ),
    ["42501"],
  );
  ...
  for (const role of [
    ...
    await expectSqlError(
      `default-privilege-denied:${role}`,
      () => roleQuery(role, "SELECT * FROM ops.ungranted_canary"),
      ["42501"],
    );
  }
  ...
  const caseId = (
    await client.query(
      "INSERT INTO editorial.cases(title) VALUES('Runtime response canary') RETURNING id",
    )
  ).rows[0].id;
  ...
  assert(
    ...
    `initial response draft version is ${draft.version}`,
  );
  record("response-draft-create-from-zero", {
  ...
  await expectSqlError("response-draft-nonzero-create-rejected", async () => {
    const otherCase = (
      await client.query(
        "INSERT INTO editorial.cases(title) VALUES('Other response canary') RETURNING id",
      )
    ).rows[0].id;
    const otherRequest = (
      await client.query(
    ...
    await client.query(
      "INSERT INTO intake.response_access_tokens(response_request_id,token_hash,expires_at,verified_at) VALUES($1,$2,clock_timestamp()+interval '1 day',clock_timestamp())",
      [otherRequest, otherToken],
    );
    await roleQuery(
      "gurine_submission_api",
      "SELECT * FROM intake.save_response_draft($1::char(64),3,decode('01','hex'),'{}',clock_timestamp()+interval '1 day')",
      [otherToken],
    );
  }, ["40001"]);
  ...
  `,
      [tokenHash, "b".repeat(64), "c".repeat(64)],
    )
  ).rows[0].id;
  const responseState = (
    await client.query(
  ...
  assert(
    responseState.status === "SUBMITTED" &&
      Number(responseState.version) === 2 &&
      responseState.token_revoked &&
      responseState.submission_count === 1,
    `response terminal state mismatch: ${JSON.stringify(responseState)}`,
  );
  record("single-response-submission", { submissionId, ...responseState });

  await expectSqlError(
    "duplicate-response-submit-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  `,
        [tokenHash, "d".repeat(64), "e".repeat(64)],
      ),
    ["28000", "55000", "23505"],
  );

  await expectSqlError(
    "response-submission-unique-constraint",
    () =>
      client.query(
        `
  ...
  `,
        [responseRequestId, "f".repeat(64), "9".repeat(64)],
      ),
    ["23505"],
  );
  ...
  const closedCase = (
    await client.query(
      "INSERT INTO editorial.cases(title) VALUES('Closed response canary') RETURNING id",
    )
  ).rows[0].id;
  const closedRequest = (
    await client.query(
  ...
  await client.query(
    "INSERT INTO intake.response_access_tokens(response_request_id,token_hash,expires_at,verified_at) VALUES($1,$2,clock_timestamp()+interval '1 day',clock_timestamp())",
    [closedRequest, closedToken],
  );
  await expectSqlError(
    "closed-response-request-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        "SELECT * FROM intake.save_response_draft($1::char(64),0,decode('00','hex'),'{}',clock_timestamp()+interval '1 day')",
        [closedToken],
      ),
    ["55000", "28000"],
  );
  ...
  const expiredCase = (
    await client.query(
      "INSERT INTO editorial.cases(title) VALUES('Expired response canary') RETURNING id",
    )
  ).rows[0].id;
  ...
  await client.query(
    "INSERT INTO intake.response_access_tokens(response_request_id,token_hash,expires_at,verified_at) VALUES($1,$2,clock_timestamp()+interval '1 day',clock_timestamp())",
    [expiredRequest, expiredToken],
  );
  await expectSqlError(
    "expired-response-request-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        "SELECT * FROM intake.save_response_draft($1::char(64),0,decode('00','hex'),'{}',clock_timestamp()+interval '1 day')",
  ...
  try {
    ...
    await client.query(
      "SELECT intake.get_response_access_status($1::char(64))",
      [tokenHash],
    );
    await client.query("COMMIT");
  } catch (error) {
  ...
  assert(
    auditExport.status === "QUEUED" && Number(auditExport.version) === 1,
    `audit export creation mismatch: ${JSON.stringify(auditExport)}`,
  );
  const auditExportRunning = (
    await roleQuery(
      "gurine_workflow_worker",
      `
  ...
  assert(
    auditExportRunning.status === "RUNNING" &&
      Number(auditExportRunning.version) === 2,
    `audit export RUNNING transition mismatch: ${JSON.stringify(auditExportRunning)}`,
  );
  const auditExportReady = (
    await roleQuery(
      "gurine_workflow_worker",
      `
    UPDATE ops.audit_exports
  ...
  `,
      [auditExport.id, "f".repeat(64)],
    )
  ).rows[0];
  assert(
    auditExportReady.status === "READY" &&
      Number(auditExportReady.version) === 3 &&
      auditExportReady.object_key === "runtime/audit-export.jsonl" &&
      auditExportReady.content_sha256.trim() === "f".repeat(64),
    `audit export READY transition mismatch: ${JSON.stringify(auditExportReady)}`,
  );
  const auditExportPrivileges = (
    await client.query(`
  ...
  assert(
    !auditExportPrivileges.public_insert &&
      !auditExportPrivileges.submission_insert &&
      !auditExportPrivileges.auditor_update,
  ...
  const legalHoldSnapshotId = (
    ...
    RETURNING id
  `,
      [legalHoldCase.id, legalHoldCase.version, "9".repeat(64), userId],
    )
  ).rows[0].id;
  const legalHoldResult = await roleQuery(
    "gurine_control_api",
    `
    WITH guarded AS (
  ...
  `,
    [
      legalHoldCase.id,
      Number(legalHoldCase.version),
      legalHoldSnapshotId,
      userId,
    ],
  );
  assert(
    legalHoldResult.rowCount === 1,
    "legal hold placement did not insert exactly one row",
  );
  assert(
    Number(legalHoldResult.rows[0].case_version) === 2 &&
      Number(legalHoldResult.rows[0].legal_hold_version) === 1,
    `legal hold placement version mismatch: ${JSON.stringify(legalHoldResult.rows[0])}`,
  );
  const legalHoldStale = await roleQuery(
    "gurine_control_api",
    `
  ...
  assert(
    legalHoldStale.rowCount === 0,
    "stale legal hold case version unexpectedly mutated the case",
  );
  ...
  try {
    ...
    await client.query(
      `UPDATE editorial.legal_holds SET reason='tampered' WHERE id=$1`,
      [legalHoldResult.rows[0].legal_hold_id],
    );
    await client.query("ROLLBACK");
  } catch (error) {
    await client.query("ROLLBACK");
    assert(
      error.code === "42501",
      `legal hold update expected 42501, got ${error.code}: ${error.message}`,
    );
    legalHoldUpdateDenied = true;
  }
  const legalHoldPrivileges = (
    await client.query(`
  ...
  assert(
    legalHoldUpdateDenied &&
      !legalHoldPrivileges.can_update &&
      !legalHoldPrivileges.can_delete,
  ...
  const sessionId = (
    ...
    RETURNING id
  `,
      [userId, sessionTokenHash, csrfHash1],
    )
  ).rows[0].id;
  const rotated = (
    await roleQuery(
  ...
  `,
      [sessionTokenHash, csrfHash1, csrfHash2],
    )
  ).rows[0].rotated;
  assert(rotated, "valid CSRF rotation returned false");
  const sessionRow = (
    await client.query("SELECT csrf_token_hash FROM ops.sessions WHERE id=$1", [
      sessionId,
    ])
  ).rows[0];
  assert(
    sessionRow.csrf_token_hash.trim() === csrfHash2,
    "CSRF hash was not rotated",
  );
  ...
  const staleRotation = (
    ...
    SELECT ops.rotate_session_csrf($1::char(64),$2::char(64),$3::char(64)) AS rotated
  `,
      [sessionTokenHash, csrfHash1, "4".repeat(64)],
    )
  ).rows[0].rotated;
...
`,
      [sessionId, actionDigest, idempotencyKeyHash, authorizationTokenHash],
    )
  ).rows[0].id;
  ...
  `,
        [authorizationTokenHash, sessionId, actionDigest, idempotencyKeyHash],
      )
    ).rows[0];
    assert(
      claimed.authorization_id === authorizationId &&
        Number(claimed.issue_number) === issue &&
        Number(claimed.remaining_issues) === 3 - issue,
      `step-up authorization issue ${issue} mismatch`,
    );
  }
  record("step-up-authorization-three-assertion-issues", { authorizationId });
  await expectSqlError(
    "step-up-authorization-fourth-issue-rejected",
    () =>
      roleQuery(
        "gurine_identity_api",
        `
...
`,
  ...
  await expectSqlError(
    "control-api-step-up-authorization-claim-denied",
    () =>
      roleQuery(
        "gurine_control_api",
        `
...
`,
  ...
  const approveResult = await roleQuery(
    "gurine_control_api",
    `
    WITH locked AS (
  ...
  assert(
    approveResult.rowCount === 1 && Number(approveResult.rows[0].version) === 2,
    "schema mapping approve concurrency failed",
  );
  const approveStale = await roleQuery(
    "gurine_control_api",
    `
  ...
  assert(
    approveStale.rowCount === 0,
    "stale schema mapping approve unexpectedly mutated the drift",
  );
  ...
  const rejectResult = await roleQuery(
    "gurine_control_api",
    `
    WITH locked AS (
      SELECT id FROM ops.schema_drifts WHERE id=$1 AND version=$2 AND status='OPEN' FOR UPDATE
  ...
  assert(
    rejectResult.rowCount === 1 && Number(rejectResult.rows[0].version) === 2,
    "schema mapping reject concurrency failed",
  );
  const rejectedMapping = (
    await client.query(
      `SELECT status FROM ops.schema_mappings WHERE schema_drift_id=$1 AND mapping_version=3`,
      [rejectDrift.id],
    )
  ).rows[0];
  assert(
    rejectedMapping.status === "REJECTED",
    "selected schema mapping was not rejected",
  );
  const rejectStale = await roleQuery(
    "gurine_control_api",
    `UPDATE ops.schema_drifts SET version=version+1 WHERE id=$1 AND version=1 RETURNING version`,
    [rejectDrift.id],
  );
  assert(
    rejectStale.rowCount === 0,
    "stale schema mapping reject unexpectedly mutated the drift",
  );
  record("schema-mapping-reject-concurrency");

  await client.query(
    `INSERT INTO ops.queue_controls(queue_name,state,version) VALUES('runtime-queue','RUNNING',1)`,
  );
  const queuePause = await roleQuery(
    "gurine_control_api",
    `UPDATE ops.queue_controls SET state='PAUSED_NEW',version=version+1 WHERE queue_name=$1 AND version=$2 RETURNING version`,
    ["runtime-queue", 1],
  );
  assert(
    queuePause.rowCount === 1 && Number(queuePause.rows[0].version) === 2,
    "queue_name concurrency predicate failed",
  );
  const queueStale = await roleQuery(
    "gurine_control_api",
    `UPDATE ops.queue_controls SET version=version+1 WHERE queue_name=$1 AND version=1 RETURNING version`,
    ["runtime-queue"],
  );
  assert(queueStale.rowCount === 0, "stale queue update succeeded");
  const sourcePause = await roleQuery(
    "gurine_control_api",
    `UPDATE ops.source_registry SET enabled=false,version=version+1 WHERE source_id=$1 AND version=1 RETURNING version`,
    ["runtime-concurrency-source"],
  );
  assert(
    sourcePause.rowCount === 1 && Number(sourcePause.rows[0].version) === 2,
    "source_id concurrency predicate failed",
  );
  const sourceStale = await roleQuery(
    "gurine_control_api",
    `UPDATE ops.source_registry SET version=version+1 WHERE source_id=$1 AND version=1 RETURNING version`,
    ["runtime-concurrency-source"],
  );
  ...
  const serviceFirst = (
    await roleQuery(
      "gurine_identity_api",
      `SELECT ops.consume_assertion_jti('SERVICE',$1::uuid,'review-console','identity-api',clock_timestamp()+interval '30 seconds',$2::char(64)) AS consumed`,
      [serviceJti, requestDigest],
    )
  ).rows[0].consumed;
  const serviceReplay = (
    await roleQuery(
  ...
  assert(
    serviceFirst && !serviceReplay,
    "service assertion replay guard failed",
  );
  ...
  const actorFirst = (
    await roleQuery(
      "gurine_control_api",
      `SELECT ops.consume_assertion_jti('ACTOR',$1::uuid,'identity-api','control-api',clock_timestamp()+interval '20 seconds',$2::char(64)) AS consumed`,
      [actorJti, requestDigest],
    )
  ).rows[0].consumed;
  const actorReplay = (
    await roleQuery(
  ...
  const submissionAssertionFirst = (
    await roleQuery(
      "gurine_submission_api",
      `
  ...
  `,
      [submissionServiceJti, submissionRequestDigest],
    )
  ).rows[0].consumed;
  const submissionAssertionReplay = (
    await roleQuery(
      "gurine_submission_api",
      `
    SELECT ops.consume_assertion_jti('SERVICE',$1::uuid,'public-web','submission-api',clock_timestamp()+interval '30 seconds',$2::char(64)) AS consumed
  ...
  assert(
    submissionAssertionFirst && !submissionAssertionReplay,
    "submission BFF service assertion replay guard failed",
  );
  record("submission-service-assertion-replay-rejected");

  const v13Case = (
    await client.query(
      "INSERT INTO editorial.cases(title) VALUES('v13 scoped response boundary') RETURNING id",
    )
  ).rows[0].id;
  const v13Request = (
    await client.query(
  ...
  assert(
    exchange.request_id === v13Request && exchange.session_id,
    "response magic token exchange failed",
  );
  await expectSqlError(
    "response-magic-token-replay-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  `,
      [v13Pending, "6".repeat(64)],
    )
  ).rows[0];
  assert(
    Number(failedOtp.remaining_attempts) === 4 && failedOtp.session_id === null,
    "response OTP failure accounting mismatch",
  );
  const verified = (
    await roleQuery(
      "gurine_submission_api",
  ...
  `,
      [v13Pending, v13Active],
    )
  ).rows[0];
  assert(
    verified.request_id === v13Request && verified.session_id,
    "response OTP promotion failed",
  );
  await expectSqlError(
    "response-pending-session-reuse-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  assert(
    Number(scopedDraft.version) === 1,
    "scoped response draft creation failed",
  );
  ...
  await client.query(
    `UPDATE intake.response_attachments SET scan_status='CLEAN' WHERE id=$1`,
    [responseAttachment],
  );
  const scopedPreview = (
    await roleQuery(
  ...
  assert(
    scopedPreview.attachments.length === 1,
    "scoped response preview missing attachment",
  );
  record("response-scoped-draft-and-attachment");

  const otherCaseV13 = (
    await client.query(
      "INSERT INTO editorial.cases(title) VALUES('v13 other scoped response') RETURNING id",
    )
  ).rows[0].id;
  const otherRequestV13 = (
    await client.query(
      `
    INSERT INTO editorial.response_requests(case_id,party_type,party_name,recipient_email_hash,recipient_email_encrypted,questions,requested_publication_scope,due_at,sent_at,status,created_by)
  ...
  `,
      [otherCaseV13, actorId],
    )
  ).rows[0].id;
  const otherActive = "9".repeat(64);
  await client.query(
    `INSERT INTO intake.submission_sessions(token_hash,session_kind,scope_type,scope_id,bff_issuer,expires_at) VALUES($1,'RESPONSE_ACTIVE','RESPONSE_REQUEST',$2,'response-portal',clock_timestamp()+interval '12 hours')`,
    [otherActive, otherRequestV13],
  );
  await expectSqlError(
    "response-attachment-cross-session-idor-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  `,
        [otherActive, responseAttachment],
      ),
    ["P0002"],
  );
  ...
  `,
      [v13Active, "a".repeat(64), v13Receipt, v13ReceiptSession],
    )
  ).rows[0];
  assert(
    scopedSubmission.submission_id && scopedSubmission.receipt_session_id,
    "scoped response submit failed",
  );
  await expectSqlError(
    "response-active-session-reuse-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  assert(
    responseReceipt.id === scopedSubmission.submission_id,
    "response receipt session scope mismatch",
  );
  ...
  assert(
    Number(correctionSaved.version) === 2,
    "correction draft version did not advance",
  );
  ...
  `,
      [correctionSessionHash, "e".repeat(64)],
    )
  ).rows[0].id;
  await roleQuery(
    "gurine_submission_api",
  ...
  await client.query(
    `UPDATE intake.correction_draft_attachments SET scan_status='CLEAN' WHERE id=$1`,
  ...
  assert(
    correctionPreview.attachments.length === 1,
    "correction preview missing attachment",
  );
  ...
  `,
      [correctionSessionHash, correctionReceiptHash, correctionReceiptSession],
    )
  ).rows[0];
  assert(
    correctionSubmit.request_id && correctionSubmit.receipt_session_id,
    "correction submit failed",
  );
  ...
  `,
      [
        correctionStart.draft_id,
        correctionSessionHash,
        correctionSubmit.request_id,
      ],
    )
  ).rows[0];
  assert(
    correctionState.draft_count === 0 &&
      correctionState.draft_session_status === "CONSUMED" &&
      correctionState.attachment_count === 1,
    `correction terminal state mismatch: ${JSON.stringify(correctionState)}`,
  );
  await expectSqlError(
    "correction-draft-session-reuse-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  `,
      [correctionReceiptSession],
    )
  ).rows[0].receipt;
  assert(
    correctionReceipt.id === correctionSubmit.request_id,
    "correction receipt scope mismatch",
  );
  ...
  await expectSqlError(
    "correction-attachment-cross-session-idor-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  `,
        [otherCorrectionHash, correctionAttachment],
      ),
    ["P0002"],
  );
  ...
  `,
      [subscriptionVerify, subscriptionManageToken, subscriptionPending],
    )
  ).rows[0];
  ...
  `,
      [subscriptionVerify, subscriptionActive],
    )
  ).rows[0];
  assert(
    subscriptionVerified.subscription_id === subscription.subscription_id,
    "subscription verification scope mismatch",
  );
  const pendingState = (
    await client.query(
      `SELECT status FROM intake.submission_sessions WHERE token_hash=$1`,
      [subscriptionPending],
    )
  ).rows[0].status;
  assert(
    pendingState === "CONSUMED",
    "pending subscription session not consumed",
  );
  ...
  assert(
    subscriptionProjection.id === subscription.subscription_id,
    "subscription session projection mismatch",
  );
  const unsubscribed = (
    await roleQuery(
      "gurine_submission_api",
  ...
  assert(
    unsubscribed === subscription.subscription_id,
    "subscription unsubscribe failed",
  );
  await expectSqlError(
    "subscription-session-reuse-rejected",
    () =>
      roleQuery(
        "gurine_submission_api",
        `
  ...
  assert(
    !newTablePrivileges.session_select &&
      !newTablePrivileges.session_insert &&
      !newTablePrivileges.correction_update &&
      newTablePrivileges.assertion_execute,
  ...
  assert(
    roles.length >= 13 &&
      roles.every((row) => !row.rolsuper && !row.rolbypassrls),
    `unsafe runtime role flags: ${JSON.stringify(roles)}`,
  );
  record("runtime-roles-no-superuser-or-bypassrls", {
