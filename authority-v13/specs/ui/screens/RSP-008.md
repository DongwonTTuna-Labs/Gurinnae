...
만료·취소·사용 완료·오류를 구분하고 안전한 지원 경로를 제공한다.
...
|---:|---|---|---|---|
| 1 | `status` | 접근 상태 | `StatusAndRevisionHeader` | 과도한 token 정보 없이. |
| 2 | `impact` | 영향 | `StructuredContentSection` | draft/submit 여부. |
| 3 | `actions` | 다음 행동 | `StructuredContentSection` | 새 링크 요청·담당자. |
| 4 | `security` | 보안 | `StructuredContentSection` | 의심 링크 신고. |
...
|---|---|---|---|---|---|
| `request-new-link` | 새 링크 요청 안내 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `contact-owner` | 담당자 문의 | `NAVIGATION` | `NONE` | `none` | `local-only` |

Primary action: `request-new-link`
...
- token 존재 여부 과다 누출
- internal ID/stack trace

## Implementation files

- Page: `apps/response-portal/src/routes/respond/unavailable/+page.svelte`
- Server load/actions: `apps/response-portal/src/routes/respond/unavailable/+page.server.ts`
- Layout: `apps/response-portal/src/routes/+layout.server.ts`
- Error boundary: `apps/response-portal/src/routes/+error.svelte`
- View model: `apps/response-portal/src/lib/view-models/rsp-008.ts`
- E2E: `tests/e2e/screens/rsp-008.spec.ts`
- Visual: `tests/visual/rsp-008.spec.ts`
