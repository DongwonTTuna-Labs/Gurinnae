...
제출 ID·시각·항목·다음 절차를 확인하고 영수증을 보관한다.
...
|---:|---|---|---|---|
| 1 | `receipt` | 접수 결과 | `DataCollection` | receipt·timestamp. |
| 2 | `summary` | 제출 요약 | `GuidedFormSection` | 질문·파일 수·consent. |
| 3 | `next` | 다음 절차 | `DecisionReviewPanel` | 검토·연락. |
| 4 | `download` | 영수증 | `StructuredContentSection` | 보관. |
...
|---|---|---|---|---|---|
| `download-receipt` | 영수증 다운로드 | `DOWNLOAD` | `NONE` | `none` | `local-only` |
| `submit-supplement` | 추가자료 안내 | `NAVIGATION` | `NONE` | `none` | `local-only` |
...
|---|---|---|---|---|---|
| `exchangeResponseReceiptToken` | `submission-api` | `POST` | `/v1/submission-session/response-receipt:exchange` | `exchangeResponseReceiptTokenRequest` | `ExchangeSubmissionSessionResult` |
| `getResponseReceipt` | `submission-api` | `GET` | `/v1/response-receipt` | `getResponseReceiptQuery` | `ResponseReceiptResponse` |
...
- Page: `apps/response-portal/src/routes/respond/receipt/+page.svelte`
- Server load/actions: `apps/response-portal/src/routes/respond/receipt/+page.server.ts`
- Layout: `apps/response-portal/src/routes/+layout.server.ts`
- Error boundary: `apps/response-portal/src/routes/+error.svelte`
- View model: `apps/response-portal/src/lib/view-models/rsp-006.ts`
- E2E: `tests/e2e/screens/rsp-006.spec.ts`
- Visual: `tests/visual/rsp-006.spec.ts`
