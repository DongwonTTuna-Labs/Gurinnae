...
모든 데이터 source의 상태·owner·sync·schema·quarantine·영향 규칙을 관리한다.
...
|---:|---|---|---|---|
| 1 | `health` | 상태 요약 | `StatusAndRevisionHeader` | healthy/stale/incident. |
| 2 | `filters` | 필터 | `UnifiedSearch` | owner/type/status. |
| 3 | `registry` | source 목록 | `DataCollection` | sync·schema·lag·quarantine. |
| 4 | `impact` | 영향 | `CoverageStatement` | rules/public freshness. |
| 5 | `ownership` | owner/runbook | `StructuredContentSection` | 연락·SLA. |
...
|---|---|---|---|---|---|
| `open-source` | source 운영 보기 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `ack-incident` | incident 확인 | `COMMAND` | `ACTIVE_SESSION` | `sources.operate` | `acknowledgeSourceIncident` |
| `pause-source` | source 일시 중지 | `DESTRUCTIVE_CONFIRMATION` | `RECENT_SESSION` | `sources.operate` | `pauseSource` |

Primary action: `ack-incident`

## Data contracts

| Operation | API | Method | Path | Request Schema | Response Schema |
|---|---|---|---|---|---|
| `listInternalSources` | `control-api` | `GET` | `/v1/internal/queries/list-internal-sources` | `listInternalSourcesQuery` | `InternalSourcesPage` |
| `acknowledgeSourceIncident` | `control-api` | `POST` | `/v1/internal/commands/acknowledge-source-incident` | `acknowledgeSourceIncidentRequest` | `acknowledgeSourceIncidentReceipt` |
...
- `internal.source_registry.viewed`
- `internal.source.opened`
- `internal.source.paused`

본문·magic token·scoped session·개인정보·첨부파일명·근거 원문은 analytics payload에 포함하지 않는다.

## 금지

- public status와 internal config 혼합
- API key 노출

## Implementation files

- Page: `apps/review-console/src/routes/internal/sources/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/sources/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/src-001.ts`
- E2E: `tests/e2e/screens/src-001.spec.ts`
- Visual: `tests/visual/src-001.spec.ts`
