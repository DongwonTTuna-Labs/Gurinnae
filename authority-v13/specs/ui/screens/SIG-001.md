...
새 이상 신호를 rule·source·quality·blocker로 정렬해 검토 대상을 선택한다.

## 이 화면이 즉시 답해야 하는 질문

- 어떤 신호가 오래되거나 blocker가 없는가?
- 이 priority는 왜 정해졌는가?
- 누가 담당하는가?
...
|---:|---|---|---|---|
| 1 | `scope` | queue 상태 | `StatusAndRevisionHeader` | SLA·count. |
| 2 | `filters` | 필터 | `UnifiedSearch` | rule/source/quality/state. |
| 3 | `table` | 신호 | `ComparisonWorkbench` | metric·cohort·freshness·assignment. |
| 4 | `bulk` | 제한된 bulk | `StructuredContentSection` | assign/label. |
...
|---|---|---|---|---|---|
| `open-signal` | 신호 검토 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `assign` | 담당자 지정 | `COMMAND` | `ACTIVE_SESSION` | `signals.triage` | `assignSignal` |
...
|---|---|---|---|---|---|
| `listSignals` | `control-api` | `GET` | `/v1/internal/queries/list-signals` | `listSignalsQuery` | `SignalsPage` |
| `assignSignal` | `control-api` | `POST` | `/v1/internal/commands/assign-signal` | `assignSignalRequest` | `assignSignalReceipt` |
...
- `internal.signal_queue.viewed`
- `internal.signal.opened`
- `internal.signal.assigned`
...
- AI confidence 기본 열
- bulk corruption/dismiss
- 공개 priority

## Implementation files

- Page: `apps/review-console/src/routes/internal/signals/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/signals/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/sig-001.ts`
- E2E: `tests/e2e/screens/sig-001.spec.ts`
- Visual: `tests/visual/sig-001.spec.ts`
