...
role별 capability·scope·conflict·사용자 수·review 주기를 검토한다.
...
|---:|---|---|---|---|
| 1 | `catalog` | role catalog | `StatusAndRevisionHeader` | description/status. |
| 2 | `capabilities` | capability matrix | `StructuredContentSection` | read/write/high impact. |
| 3 | `conflicts` | SoD | `AccessManagementPanel` | conflicting roles. |
| 4 | `grants` | grant stats | `StructuredContentSection` | count/expiry. |
| 5 | `review` | 정기 검토 | `DecisionReviewPanel` | owner/last review. |
| 6 | `history` | 변경 이력 | `RevisionTimeline` | version. |
...
|---|---|---|---|---|---|
| `open-role` | 역할 상세 | `NAVIGATION` | `NONE` | `none` | `local-only` |
| `propose-change` | 역할 변경 제안 | `COMMAND` | `STEP_UP` | `roles.grant` | `proposeRoleDefinitionChange` |
| `export-matrix` | 권한 matrix 다운로드 | `DOWNLOAD` | `NONE` | `audit.read` | `local-only` |
...
|---|---|---|---|---|---|
| `listRoleDefinitions` | `control-api` | `GET` | `/v1/internal/queries/list-role-definitions` | `listRoleDefinitionsQuery` | `RoleDefinitionsPage` |
| `proposeRoleDefinitionChange` | `control-api` | `POST` | `/v1/internal/commands/propose-role-definition-change` | `proposeRoleDefinitionChangeRequest` | `proposeRoleDefinitionChangeReceipt` |
...
- `internal.roles.viewed`
- `internal.role_change.proposed`
- `internal.role_matrix.exported`

본문·magic token·scoped session·개인정보·첨부파일명·근거 원문은 analytics payload에 포함하지 않는다.

## 금지

- role in-place 무감사 변경
- capability 설명 없음

## Implementation files

- Page: `apps/review-console/src/routes/internal/admin/roles/+page.svelte`
- Server load/actions: `apps/review-console/src/routes/internal/admin/roles/+page.server.ts`
...
- View model: `apps/review-console/src/lib/view-models/adm-003.ts`
- E2E: `tests/e2e/screens/adm-003.spec.ts`
- Visual: `tests/visual/adm-003.spec.ts`
