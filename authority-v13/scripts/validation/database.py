...
def validate(root: Path, result: Validation) -> None:
    ...
    result.require((root/'scripts/compare_postgres_runtime.py').is_file() and (root/'scripts/authority_tree_digest.py').is_file(), 'reproducible runtime comparison tooling missing')
    result.stats.update({
